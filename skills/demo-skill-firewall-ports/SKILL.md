...
- 演示或应急处置：对外只保留 **22（SSH）** 与 **80（HTTP）**

**不适用**：未确认业务依赖就直接全阻断；生产变更窗口外的批量改防火墙；需要 HTTPS 时未评估 **443**（本 Skill 演示口径默认不放行 443）。
...
- 80 是否为真实业务入口；若实际走 443，需用户明确是否调整演示策略
- 数据库、中间件、调试端口（如 3306、6379、8080、9090）是否误暴露

### 3. 处置方案（用户确认后再执行）

推荐优先 **ufw**（Ubuntu/Debian）示例口径：
